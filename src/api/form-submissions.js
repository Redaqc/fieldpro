...
export const formSubmissionsApi = {
  getAll: async (params = {}) => {
    try {
      ...
      const response = await apiClient.get(`/form-submissions?${queryParams}`);
  ...
  create: async (data) => {
    try {
      const response = await apiClient.post('/form-submissions', data);
      return response.data;
    } catch (error) {
      throw handleApiError(error);
    }
...
export default formSubmissionsApi;
