...
export const maintenanceSchedulesApi = {
  getAll: async (params = {}) => {
    try {
      ...
      const response = await apiClient.get(`/maintenance-schedules?${queryParams}`);
  ...
  update: async (id, data) => {
    try {
      const response = await apiClient.patch(`/maintenance-schedules/${id}`, data);
      return response.data;
    } catch (error) {
      throw handleApiError(error);
    }
  ...
  getStatistics: async () => {
    try {
      const response = await apiClient.get('/maintenance-schedules/stats/overview');
      return response.data;
    } catch (error) {
      throw handleApiError(error);
    }
...
export default maintenanceSchedulesApi;
