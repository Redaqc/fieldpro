...
export const supplierInvoicesApi = {
  getAll: async (params = {}) => {
    try {
      ...
      const response = await apiClient.get(`/supplier-invoices?${queryParams}`);
  ...
  delete: async (id) => {
    try {
      const response = await apiClient.delete(`/supplier-invoices/${id}`);
      return response.data;
    } catch (error) {
      throw handleApiError(error);
    }
  ...
  getStatistics: async () => {
    try {
      const response = await apiClient.get('/supplier-invoices/stats/overview');
      return response.data;
    } catch (error) {
      throw handleApiError(error);
    }
